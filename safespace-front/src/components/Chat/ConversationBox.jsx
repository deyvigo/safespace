...
export const ConversationBox = ({ selectedConversationId, onSelectConversation, conversations, updateConversations }) => {
  ...
  return (
    <div className="w-full lg:min-w-[300px] lg:max-w-[350px] flex flex-col bg-white border-r border-slate-200 overflow-hidden">
      ...
      <div className="flex-1 flex flex-row lg:flex-col overflow-auto">
        {conversations.map(
          ({ conversation_id, name, last_name, username, last_message }) => (
            <ConversationItem
              name={`${name} ${last_name}`}
              key={conversation_id}
